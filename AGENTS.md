...
<!--
===============================================================================
THIS SECTION IS DELIBERATELY MUTABLE
===============================================================================

Agents SHOULD update this section.

Add only durable project-specific knowledge that another capable agent would
otherwise have to rediscover.

Prefer concise, executable facts.

Do not duplicate information that is obvious from the repository.

Delete or update stale mutable instructions.

For large repositories, create nested AGENTS.md files near specialized
subsystems instead of turning this section into a giant encyclopedia.
...
| App | `src/app/` | Next.js App Router routes | — |
| Lib | `src/lib/` | preview-auth, safe-redirect, product-status, structured-data | — |
| Visuals | `src/components/visuals/` | HeroMap SVG system | — |
| Scripts | `scripts/` | preview-hash, jsonld-hash helpers | — |
| Golden master | `.archive-static-golden-master/` | pre-migration static site (trace-excluded) | — |
...
```sh
yarn vitest run src/lib/safe-redirect.test.ts
```
...
```sh
NEXT_PUBLIC_JSONLD_HASH=$(node scripts/jsonld-hash.mjs) yarn build
```

### Full validation / Definition-of-Done command

```sh
yarn typecheck && yarn test && NEXT_PUBLIC_JSONLD_HASH=$(node scripts/jsonld-hash.mjs) yarn build && trace verify --changed
```
...
Node 22 (verified on v22.22.2); Yarn 4.18.0 pinned via `packageManager`
(4.13.0 cannot install TypeScript 7). Next.js 16.3.5, React 19.3.0,
TypeScript 7.0.2, Tailwind CSS v4.3.3, Vitest 5.0.1 + Vite 8.3.0.

---

## Configuration

`.env.local` (never committed): `PREVIEW_SESSION_SECRET` (32+ random bytes,
hex; rotation revokes all sessions), `PREVIEW_PASSWORD_HASH` (mint with
`yarn preview:hash`), `SITE_PREVIEW_GATED=true` until launch,
`NEXT_PUBLIC_JSONLD_HASH` (recompute with `node scripts/jsonld-hash.mjs`
after any structured-data edit; CSP breaks otherwise). Rate limiting lives in
the Vercel WAF dashboard (10 attempts/min/IP, observe-first), not in code.

---

## Architecture boundaries

`src/lib/product-status.ts` owns all maturity claims; copy, nav, llms.txt,
and JSON-LD read from it. `src/proxy.ts` owns gating; route handlers must not
reimplement auth. `HOMEPAGE_JSONLD` bytes own the CSP hash; page.tsx inlines
them verbatim. Server components by default; client components only for
browser state/APIs.
...
- `src/lib/product-status.ts` owns product maturity; llms.txt, JSON-LD, copy follow it.
- `src/lib/structured-data.ts` owns `HOMEPAGE_JSONLD` bytes; `NEXT_PUBLIC_JSONLD_HASH` follows it.
- Never edit `.next/` or `next-env.d.ts` by hand (Next-generated).
- `.archive-static-golden-master/` is the frozen pre-migration site; read-only reference.
...
- Next 16.3 proxy convention: `src/proxy.ts` is picked up via the functions manifest (`/_middleware` entry), not the legacy middleware manifest. An empty `middleware.manifest.middleware={}` with a `ƒ Proxy (Middleware)` build line is the healthy signal. Verified 2026-09-16 on Next 16.3.5.
- Repo root proxy.ts is silently ignored by the Next 16.3.5 builder; src/proxy.ts is required. Proven by isolated /tmp/proxytest (root ignored, src picked up, 418 PROXY-HIT at runtime).
- Yarn 4.13.0 cannot install TypeScript 7.0.2 (builtin compat/typescript patch lstat `/node_modules/typescript/lib/_tsc.js`); Yarn 4.18.0 installs cleanly. Repo pins yarn@4.18.0 for this reason.
- Yarn 4.18.0 quarantines @types/node 22.20.3 (published 2026-09-15); repo pins 22.20.2 until the quarantine lifts.
- TraceLayer TS markers must sit directly above the symbol line (no blank line, and must survive doc-comment adjacency); file-top markers attach as file-level and trip TL003. Test files with zero parsed symbols satisfy TL012 via file-level `trace:exempt reason=...`.
- JSON files use `.trace/sidecars/<path>.json` markers, not inline comments; sidecar lines must match current key lines within ±1 or TL013 returns.
- Homepage JSON-LD is byte-stable: `src/app/page.tsx` inlines `HOMEPAGE_JSONLD` verbatim and `NEXT_PUBLIC_JSONLD_HASH` carries its SHA-256. NOT currently enforced: the shipped CSP uses `script-src 'self' 'unsafe-inline' 'unsafe-eval'`, and browsers ignore `'unsafe-inline'` as soon as any hash or nonce is listed, which would break Next's hydration. Spec §40 declines site-wide nonce rendering, so the hash is computed and kept byte-stable for a future nonce/hash scheme but pins nothing today. Recompute with `node scripts/jsonld-hash.mjs` after any structured-data edit.
